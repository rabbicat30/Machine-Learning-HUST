...
#这里分别探究参数C和参数gamma对SVC分类器分类效果的影响
def svm_with_cross_validation(train_x, train_y):
    ...
    cv_scores = []
    C = [0.01,0.1,1,3,5]
    Gamma = [1e-4,1e-3,1e-2,1e-1,1,10,100]#如果要探究参数c的影响，只需要将gamma替换成C就可以
    for gamma in Gamma:
    ...
    plt.ylabel('Accuarcy')
    plt.show()
    model = SVC(gamma=best_params["gamma"])
